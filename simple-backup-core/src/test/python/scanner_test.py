...
class Test_scanner_test(unittest.TestCase):
    ...
    def test_finds_new_files(self):
        ...
        loose_file_entry = scanner.ScanResult.FileEntry(backup_id, loose_file_local_path, loose_file_mtime, loose_file_digest)
        ...
        file_in_subdir_entry = scanner.ScanResult.FileEntry(backup_id, file_in_subdir_local_path, file_in_subdir_mtime, file_in_subdir_digest)
    ...
    def test_finds_modified_files(self):
        ...
        loose_file_digest = b'\xe8\x0bP\x17\t\x89P\xfcX\xaa\xd8<\x8c\x14\x97\x8e'
        loose_file_entry = scanner.ScanResult.FileEntry(backup_id, loose_file_local_path, loose_file_mtime_updated, loose_file_digest)
        ...
        file_in_subdir_entry = scanner.ScanResult.FileEntry(backup_id, file_in_subdir_local_path, file_in_subdir_mtime_updated, file_in_subdir_digest_updated)
    ...
    def test_moved_files_are_marked_as_modified(self):
        db = file_db.FileDB(os.path.join(self.tmpdir, 'test.db'))
        backup_dir = os.path.join(self.tmpdir, 'to_backup')
        os.mkdir(backup_dir)
        backup_id = db.add_backup('local', 'test_backup', backup_dir)
        ...
        self.assertCountEqual(changes.added, [])
        self.assertCountEqual(changes.modified, [ loose_file_entry ])
        self.assertCountEqual(changes.removed, [])
